...
fit <- Maaslin2(
  ...
  min_abundance = 0,
  min_prevalence = 0.65, # keep features present in at least 24 samples
...
fit_sig <- filter(fit_results, qval <= 0.05) %>%
...
In total, MaAsLin2 identified `r n_distinct(fit_sig$feature)` deferentially abundant features.

```{r, fig.width=5.5, fig.height=10}
hmp <- plot_heatmap(
  maaslin2_output = fit_results,
  qval_cutoff = 0.05,
  first_n = n_distinct(fit_sig$feature), # number of features to display
...
fig5_main <- grid.grabExpr(
  draw(
    hmp, 
    padding = unit(c(6, 2, 2, 0), "mm"), # bottom, left, top and right
    heatmap_legend_side = "top"
   )
  ) 
